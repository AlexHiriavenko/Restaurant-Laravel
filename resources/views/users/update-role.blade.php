    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <form action="{{ route('users.update-role') }}" method="GET" class="mb-6">
                <div class="mb-4">
                    <label for="searchText" class="block text-sm font-medium text-gray-700 mb-2">Search by Email or
                        Name</label>
                    <input type="text" id="searchText" name="searchText" value="{{ old('searchText', $searchText) }}"
                    ...
                    <table class="min-w-full bg-white border border-gray-300 rounded-md">
                        <thead class="bg-gray-100">
                            <tr>
                                ...
                                <th class="px-4 py-2 text-left text-sm font-medium text-gray-700 border-b">Current Role
                                </th>
                        ...
                        <tbody>
                            @foreach ($users as $user)
                                <tr class="border-b hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-2">
                                        <form action="{{ route('users.update-role.post') }}" method="POST"
                                            class="space-y-2">
                                            @csrf
                                            @method('PATCH')
